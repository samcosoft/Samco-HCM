...
namespace Samco_HCM_Laboratory_Client.Views.Settings
{
    ...
    public partial class SettingView
    {
        ...
        private void SettingView_OnLoaded(object sender, RoutedEventArgs e)
        {
            if (SamcoSoft.LoadedSettings?.DatabaseType != HCMSettings.DatabaseTypes.NotSet)
            {
                switch (SamcoSoft.LoadedSettings?.DatabaseType)
                {
                    ...
                    case HCMSettings.DatabaseTypes.Access:
                        ...
                        break;
                    case HCMSettings.DatabaseTypes.SQLite:
                        LocalRadioButton.IsChecked = true;
                        SqLiteLocal.IsSelected = true;
                        break;
                    default:
            ...
            //Load remote database settings
            if (SamcoSoft.LoadedSettings.RemoteDatabaseType != HCMSettings.DatabaseTypes.NotSet)
            {
                switch (SamcoSoft.LoadedSettings.DatabaseType)
                {
                    case HCMSettings.DatabaseTypes.MicrosoftSQLServer:
                        RemoteMsSqlButton.IsSelected = true;
                        break;
                    case HCMSettings.DatabaseTypes.MySql:
                        RemoteMySQlButton.IsSelected = true;
                        break;
                    default:
                        throw new ArgumentOutOfRangeException();
                }
        ...
        {
            if (LocalRadioButton.IsChecked.GetValueOrDefault() && e.Value == null)
        ...
        private void BrowseClickBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (AccessLocal.IsSelected)
            {
                saveFile.Filter = "Access file (*.accdb)|*.accdb";
            }
            else if (MsSqlLocal.IsSelected)
            {
                saveFile.Filter = "Microsoft SQL|*.mdf";
            }
            else
                saveFile.Filter = "SQLite|*.db";
        ...
        private void RemoteDatabaseAddress_OnValidate(object sender, ValidationEventArgs e)
        {
            if (ServerButton.IsChecked.GetValueOrDefault() && e.Value == null)
                e.SetError("وارد نمودن آدرس پایگاه داده سرور پذیرش ضروری است.",
                    DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical);
            else
                e.SetError(null);
        }

        private void RemoteDatabaseName_OnValidate(object sender, ValidationEventArgs e)
        {
            if (ServerButton.IsChecked.GetValueOrDefault() && e.Value == null)
                e.SetError("وارد کردن نام کاربری برای اتصال به سرور ضروری است.",
                    DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical);
        ...
        private void RemoteUserName_OnValidate(object sender, ValidationEventArgs e)
        {
            if (ServerButton.IsChecked.GetValueOrDefault() && e.Value == null)
                e.SetError("وارد کردن نام کاربری برای اتصال به سرور ضروری است.",
                    DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical);
        ...
        private void RemotePassword_OnValidate(object sender, ValidationEventArgs e)
        {
            if (ServerButton.IsChecked.GetValueOrDefault() && e.Value == null)
                e.SetError("وارد کردن کلمه عبور برای اتصال به سرور ضروری است.",
                    DevExpress.XtraEditors.DXErrorProvider.ErrorType.Critical);
        ...
        private void SetDataConBtn_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (LocalRadioButton.IsChecked == true)
            {
                ...
                if (AccessLocal.IsSelected)
                {
                    if (AcsPassBx.HasValidationError)
                    {
                        MainNotify.ShowError("خطا در ارتباط با پایگاه داده",
                            "لطفاً کلمه عبور را جهت استفاده از پایگاه داده وارد کنید.");
                        return;
                    }
                ...
                if (SqLiteLocal.IsSelected)
                {

                    SamcoSoft.LoadedSettings.DatabaseType = HCMSettings.DatabaseTypes.SQLite;
                }
            }
            else
            {
                ...
                {
                    MainNotify.ShowError("خطا در ارتباط با پایگاه داده",
                        "لطفاً تمام اطلاعات خواسته شده را جهت استفاده از پایگاه داده وارد کنید.");
                    return;
                }
            ...
            if (ClientModeBtn.IsChecked.GetValueOrDefault(false))
            {
                SamcoSoft.LoadedSettings.RemoteDatabaseType = RemoteMsSqlButton.IsSelected
                    ? HCMSettings.DatabaseTypes.MicrosoftSQLServer
                    : HCMSettings.DatabaseTypes.MySql;
                ...
                {
                    MainNotify.ShowError("خطا در ارتباط با پایگاه داده سیستم پذیرش", string.Format(
                        "امکان برقراری ارتباط با پایگاه داده سیستم پذیرش وجود ندارد. ممکن است درایورهای مورد نیاز نصب نباشند و یا کلمه عبور را اشتباه وارد نموده‌اید. اطلاعات بیشتر:{0}{0}{1}",
                        '\n', errorMessage));
                }
            }

            if (SamcoAdd.LoadDatabase(SamcoSoft.LoadedSettings.ConnectionString!, ref errorMessage))
            {
                SamcoSoft.LoadedSettings.Save();
