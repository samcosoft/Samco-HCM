...
public partial class LoginView
{
    public LoginView()
    {
        ...
        {
            if (session1.Query<LabUsers>().Count() == 1) FirstUserNotification.Visibility = Visibility.Visible;
            foreach (var user in session1.Query<LabUsers>().ToList().Where(user => string.IsNullOrEmpty(user.realname)))
            {
                user.realname = user.username;
                user.Save();
            }
            UserBx.ItemsSource = session1.Query<LabUsers>().Select(x => new LoginUser(x)).ToList();
        }
    ...
    private void LoginBtn_Click(object sender, RoutedEventArgs e)
    {
        ...
        using var session1 = new Session();
        var selUser = session1.FindObject<LabUsers>(new BinaryOperator(nameof(LabUsers.username), UserBx.EditValue.ToString()));
        ...
        SamcoSoftShared.CurrentUser = new LoggedUser
        {
            Oid = selUser.Oid, Username = selUser.username, Group = selUser.group, RealName = selUser.realname,
    ...
    private class LoginUser(LabUsers user)
    {
