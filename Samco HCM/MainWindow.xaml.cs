...
public partial class MainWindow
{
    ...
    private void MainWindow_OnContentRendered(object sender, EventArgs e)
    {
        ...
        using (var session1 = new Session())
        {
            ...
            if (!usr.Any())
            {
                var adminUser = new Users(session1) { username = "admin", password = BCrypt.Net.BCrypt.EnhancedHashPassword("admin"), group = "O", realname = "مدیر سیستم" };
                adminUser.Save();
            }
        }
    ...
    private void CreateLoadSetting(string settingDir)
    {
        ...
        if (File.Exists(settingFile))
        {
            ...
            catch (Exception)
            {
                ...
                SamcoSoftShared.LoadedSettings = new HCMSettings(SettingFileName);
            }
        }
        else
        {
